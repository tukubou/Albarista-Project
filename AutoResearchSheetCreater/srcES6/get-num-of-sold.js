...
function getCasesSold(ranking, dataTable, filterCategory) {
  ...
  // ランキングでソートする
  var sortTable = _.sortBy(filteredTable, function(num){ return num[3]; });
  // Nearly BigなRowを取得
  Logger.log('ranking=');
  Logger.log(ranking);
  var maxRow = _.max(sortTable, function(v){return v[3]});
  var minRow = _.min(sortTable, function(v){return v[3]});
  if(ranking > maxRow[3]) {
